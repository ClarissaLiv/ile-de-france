...
def execute(context):
    ...
    df_trips["departure_time"] = df_trips["V2_OLDDEH"].apply(convert_time).astype(np.float)
    df_trips["arrival_time"]   = df_trips["V2_OLDARH"].apply(convert_time).astype(np.float)
